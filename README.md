...
- Remote Command：通过用户配置的 HTTPS/HTTP 命令执行网关提供 `run_command`，AI 可向远程服务器发送 shell 命令并读取 stdout/stderr。网关需接受 `POST` JSON：`command`、`cwd`、`timeout_ms`，建议使用 HTTPS、强 Token、服务器侧目录/命令白名单。
...
- 顶栏中间的 Clawd 入口会弹出缓存保活快照浮窗，可刷新服务器状态或立即同步快照。
