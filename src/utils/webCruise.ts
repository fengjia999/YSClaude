...
export const WEB_CRUISE_SYSTEM_PROMPT = [
  ...
  '工作流必须这样执行：先根据用户偏好决策要逛的平台或方向；调用 get_hotboard 获取热搜榜，用户指定平台时用 types 参数传平台 type，例如 weibo、zhihu、bilibili、douyin、juejin、ithome；从榜单里选择 1-3 个最值得看的链接；使用 webview_open 和必要的 webview_eval_js 查看候选网页内容；最终只选择 1 个话题自然回复用户。',
